...
input.forEach(element => {
    let dashIndex = element.indexOf('-');
    let spaceIndex = element.indexOf(' ');
    let min= parseInt(element.slice(0, dashIndex)); 
    let max= parseInt(element.slice(dashIndex + 1, spaceIndex)); 
    let letter= element.slice(spaceIndex + 1, spaceIndex+2); 
    let password = element.slice(spaceIndex + 4, element.length); 

    let isFirst = password[min-1] === letter;
    let isSecond = password[max-1] === letter;
    if (isFirst ? !isSecond : isSecond) {
        valid++;
    }
});
